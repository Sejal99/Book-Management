...
const Navbar = () => {
  ...
  return (
    <div className="p-4 sticky top-0 bg-black flex justify-between items-center "style={{ zIndex: 9999 }}>
      <div onClick={()=> router.push('/')} className="text-white cursor-pointer text-2xl font-bold">Booksy</div>
        <div className=" flex gap-5">
      <div onClick={()=> router.push('/my-orders')} className=" text-white cursor-pointer p-2 rounded-lg bg-red-600">My Orders</div>
      {
        token && <div onClick={handleLogout} className=" text-white cursor-pointer p-2 rounded-lg bg-red-600">Logout</div>
      }
